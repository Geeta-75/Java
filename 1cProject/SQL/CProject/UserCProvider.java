...
public class UserCProvider {
    ...
    public void printStudent(){
        ...
        int totalStWidth = (idWidth+nameWidth+rollWidth+conWidth+courseWidth+creditsWidth+17)-"| Total number of students : ".length();
    ...
    public void printStudent(int courseID){
		...
		if(students.size()!=0){
	        ...
	        int totalStWidth = (idWidth+nameWidth+rollWidth+conWidth+courseWidth+creditsWidth+12+5)-("| Total number of students enrolled in : ".length())-courseName.length();
	        String lsFormat ="-%" + idWidth + "s-%" + nameWidth + "s-%" + rollWidth + "s-%"+conWidth+"s-%"+courseWidth+"s-%"+creditsWidth+"s-%n";
	        String hFormat ="| %" + idWidth + "s | %-" + nameWidth + "s | %" + rollWidth + "s | %"+conWidth+"s | %"+courseWidth+"s | %"+creditsWidth+"s |%n";
	        String format = "| %-" + idWidth + "d | %" + nameWidth + "s | %" + rollWidth + "d | %"+conWidth+"d | %"+courseWidth+"s | %"+creditsWidth+"d |%n";
	        String tStFormat = "| Total number of students enrolled in %"+courseWidth+"s : %"+totalStWidth+"d |%n";
        ...
        System.out.printf(lsFormat,"-".repeat(idWidth + 2), "-".repeat(nameWidth + 2), "-".repeat(rollWidth + 2),"-".repeat(conWidth + 2),"-".repeat(courseWidth + 2),"-".repeat(creditsWidth + 2));
            	System.out.printf(tStFormat, courseName, count);
                System.out.printf(lsFormat,"-".repeat(idWidth + 2), "-".repeat(nameWidth + 2), "-".repeat(rollWidth + 2),"-".repeat(conWidth + 2),"-".repeat(courseWidth + 2),"-".repeat(creditsWidth + 2));
    ...
    public void printStudent(String enStatus){
    	try {
    	List<Student> students = dbManager.getStudent(cProvider.getCPID());
		int count = 0;
		String courseName = null;
		if(students.size()!=0){
	        ...
	        int rollWidth = "Roll No.".length();
	        int conWidth = 10;
	        int courseWidth = "Enrollment Status".length();
	        ...
	        int totalStWidth = (idWidth+nameWidth+rollWidth+conWidth+courseWidth+11)-("| Total number of students not enrolled in any courses".length());
	        String lsFormat ="-%" + idWidth + "s-%" + nameWidth + "s-%" + rollWidth + "s-%"+conWidth+"s-%"+courseWidth+"s-%n";
	        String hFormat ="| %" + idWidth + "s | %-" + nameWidth + "s | %" + rollWidth + "s | %"+conWidth+"s | %"+courseWidth+"s |%n";
	        String format = "| %-" + idWidth + "d | %" + nameWidth + "s | %" + rollWidth + "d | %"+conWidth+"d | %"+courseWidth+"s |%n";
	        String tStFormat = "| Total number of students not enrolled in any courses : %"+totalStWidth+"d |%n";

	        System.out.printf(lsFormat,"-".repeat(idWidth + 2), "-".repeat(nameWidth + 2), "-".repeat(rollWidth + 2),"-".repeat(conWidth + 2),"-".repeat(courseWidth + 2));
	        System.out.printf(hFormat, "st.ID", "Student Name", "Roll No.","Contact","Enrollment Status");
	        System.out.printf(lsFormat,"-".repeat(idWidth + 2), "-".repeat(nameWidth + 2), "-".repeat(rollWidth + 2),"-".repeat(conWidth + 2),"-".repeat(courseWidth + 2));


    	for (Student student : students) {
    		if(student.getStCourseID()==404) {
    	        System.out.printf(format, student.getStudentID(), student.getStudentName(), student.getStudentRoll(), student.getStudentCon(), student.getStudentCourse());
    	        count++;
    		}            
